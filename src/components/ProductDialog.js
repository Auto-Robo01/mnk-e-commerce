...
export default function ProductDialog( { openDialog, setCloseDialog }) {
    ...
    React.useEffect(() => {
      if (data.productsData) {
        ...
        setFilteredProducts((prevProducts) => [...prevProducts, ...data.productsData]);
        setLoader(false);
      }
    }, [data.productsData]);
  ...
  const addProducts = () => {
    ...
    for(let product in groupedSelection) {
        let objectToPush = {}
        let productSelected = products.findIndex((element) => element.id == product)
    ...
    <div>
      <React.Fragment> 
        ...
        >
          ...
          <TextField value={searchText} onChange={handleSearchChange} style={{marginLeft: '10px', marginRight: '10px'}} placeholder='Searc product'/>
          <hr style={{ border: '1px solid lightgrey', margin: '10px 0' }}/>
          <DialogContent onScroll={handleScroll}  style={{ maxHeight: '400px', overflow: 'auto' }}>
                  <Box>
            <div style={{minWidth: '550px', minHeight: '400px'}}>
              {loader 
              ?     <Box sx={{ display: 'flex', marginLeft: '230px', marginTop: '185px' }}>
                         <CircularProgress />
                     </Box> 
              :  <DialogProducts  products={filteredProducts} onSelectionChange={handleSelectionChange} addProducts = {addProducts} searchText={searchText}/>}
            </div>
              </Box>
          </DialogContent>
          <DialogActions>
