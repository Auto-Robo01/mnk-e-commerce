  ...
  return (
    <div>
      <div className="product-row">
        ...
        {!isVariant && <div className="serial-number">{index + 1}</div>}
        ...
        <div className="remove-product-icon">
          {(rows.length > 1 || isVariant) && (
            <AiOutlineClose onClick={handleRemoveRow} />
          )}
        </div>
        {openDialog && (
      ...
      {variants?.length > 1 && (
        <div>
          <span className="show-variant-button" onClick={handleShowVariants}>
            Show Variants
            {showVariant ? <AiOutlineUp /> : <AiOutlineDown />}
          </span>
          {showVariant && (
